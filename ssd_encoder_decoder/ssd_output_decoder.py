...
def decode_detections(y_pred,
    ...
    Arguments:
        y_pred (array): SSD 的输出, 期望是 Numpy array, 形状为 `(batch_size, #boxes, #classes + 4 + 4 + 4)`, 
            ...
            `[6个类别的概率值, 预测边界框的 4 个相对坐标, anchor 的 4 个坐标, 4 个 variances 的值]`.
    ...
    Returns:
        A python list of length `batch_size` where each list element represents the predicted boxes for one image and contains a Numpy array of shape `(boxes, 6)` where each row is a box prediction for a non-background class for the respective image in the format `[class_id, confidence, xmin, ymin, xmax, ymax]`.
    '''
    ...
    n_classes = y_pred_decoded_raw.shape[-1] - 4 #  10 - 4 = 6 The number of classes is the length of the last axis minus the four box coordinates
