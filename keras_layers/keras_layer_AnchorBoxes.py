...
from tensorflow_tanxinkeji_works.Preject4_目标检测.bounding_box_utils.bounding_box_utils import convert_coordinates
...
class AnchorBoxes(Layer):
    ...
    def __init__(self,
        ...
        super(AnchorBoxes, self).__init__(**kwargs)   #引用父类,加载父类的构造方法

    def build(self, input_shape):
        self.input_spec = [InputSpec(shape=input_shape)]   #输入形状
        super(AnchorBoxes, self).build(input_shape)       #调用父类的函数
    ...
    def call(self, x, mask=None):
        ...
        size = min(self.img_height, self.img_width)  #min() 方法返回给定参数的最小值，参数可以为序列。例子：print ("min(80, 100, 1000) ： ", min(80, 100, 1000))
        wh_list = []
        for ar in self.aspect_ratios:
            if (ar == 1):
                ...
                wh_list.append((box_width, box_height))
        wh_list = np.array(wh_list)   #根据宽高比生成相应Anchor的的高度和宽度并存储在'wh_list'之中

        # 获取输入 tensor 的形状
        if K.image_data_format() == 'channels_last':
            batch_size, feature_map_height, feature_map_width, feature_map_channels = x._keras_shape
        else: # 因为只支持 TensorFlow, 下面这一句可以不要, 也不影响功能
            batch_size, feature_map_channels, feature_map_height, feature_map_width = x._keras_shape

        # 计算 Anchor 的中心点位置, 对于同一位置的Anchor虽然长宽比不同但是中心点位置是同样的.
        # 计算步长. 计算两个相邻 Anchor 的中心的纵横方向的距离，注意这里的步长是相对于原始图片的步长
        if (self.this_steps is None):
            step_height = self.img_height / feature_map_height  # 步长是相对于原始图片横方向上距离的步长
            step_width = self.img_width / feature_map_width    # 步长是相对于原始图片纵方向上距离的步长
        else:
        ...
        # 有了偏移量和步长, 计算 Anchor 的中心位置，找到中心位置是相对于原图中心点位置
        ...
        boxes_tensor[:, :, :, 2] = wh_list[:, 0] # 设置 w  #wh_list[:, 0]找出wh_list的第一列，并广播赋值给boxes_tensor
        boxes_tensor[:, :, :, 3] = wh_list[:, 1] # 设置 h  #wh_list[:, 0]找出wh_list的第二列，并广播赋值给boxes_tensor
        ...
        if self.clip_boxes:
            x_coords = boxes_tensor[:,:,:,[0, 2]]  #先转化成为(xmin, xmax, ymin, ymax)坐标格式，然后找出cx和w
            x_coords[x_coords >= self.img_width] = self.img_width - 1
            x_coords[x_coords < 0] = 0
            boxes_tensor[:,:,:,[0, 2]] = x_coords
            y_coords = boxes_tensor[:,:,:,[1, 3]]   #先转化成为(xmin, xmax, ymin, ymax)坐标格式，然后找出cy和h
        ...
        if self.normalize_coords:
            boxes_tensor[:, :, :, [0, 2]] /= self.img_width   #先转化成为(xmin, xmax, ymin, ymax)坐标格式，然后boxes_tensor[:, :, :, [0, 2]]找出cx和w
            boxes_tensor[:, :, :, [1, 3]] /= self.img_height  #先转化成为(xmin, xmax, ymin, ymax)坐标格式，然后boxes_tensor[:, :, :, [1, 3]]找出cy和h
        ...
        boxes_tensor = np.concatenate((boxes_tensor, variances_tensor), axis=-1)  #(feature_map_height, feature_map_width, n_boxes, 4)转化为(feature_map_height, feature_map_width, n_boxes, 8)

        # 在 `boxes_tensor` 前面加一个 维度, 作为 batch size 的维度
        # 拼接以后维度为 5 维, 形状为 `(batch_size, feature_map_height, feature_map_width, n_boxes, 8)（batch, 37, 40, 4, 8(4个坐标值+variances)）
        boxes_tensor = np.expand_dims(boxes_tensor, axis=0)
        boxes_tensor = K.tile(K.constant(boxes_tensor, dtype='float32'), (K.shape(x)[0], 1, 1, 1, 1))  #最后转化为tensor张量
