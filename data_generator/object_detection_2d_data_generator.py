...
from tensorflow_tanxinkeji_works.Preject4_目标检测.data_generator.object_detection_2d_image_boxes_validation_utils import BoxFilter
...
class DataGenerator:
    ...
    def  parse_csv(self,
        ...
        Arguments:
            ...
            include_classes (list, optional): Either 'all' or a list of integers containing the class IDs that
                are to be included in the random_sample (float, optional): Either `False` or a float in `[0,1]`. If this is `False`, the
                ...
                of its boxes.dataset. If 'all', all ground truth boxes will be included in the dataset.
        ...
        # 在开始之前，请确保有labels_filename和input_format
        ...
        # 删除以前可能已经解析过的数据
        self.filenames = []  #保存图像的路径（相对路径）
        self.image_ids = []  #保存图像名（不包括后缀名）
        self.labels = []  # 保存类别和坐标信息
        #注意：self.labels:
        #   本项目中的labels是不规则的列表。打印出来是一维的，但实际上可以理解为二维的，第一维度是行即每张图片一行，第二维度是列，每张图片对应的所有anchorbox的类别和坐标信息
        ...
        with open(self.labels_filename, newline='') as csvfile:
            ...
            next(csvread)  # 跳过第一行标签行
            for row in csvread:
                if self.include_classes == 'all' or int(row[self.input_format.index('class_id')].strip()) in self.include_classes: # 如果读取包含所有类别或者class_id是数据集中包含的类之一
                    box = [] #存储每个anchor box的图片名、类别、坐标，示例展示：['1478899046136829030.jpg', 5, 201, 129, 206, 135]
                    box.append(row[self.input_format.index('frame')].strip())
                    for element in self.labels_output_format:  # 在这一步，会将原始文件中坐标格式（Xmin, Xmax, Ymin, Ymax）转化成为（Xmin, Ymin, Xmax, Ymax）中心坐标点格式
                        box.append(int(row[self.input_format.index(element)].strip()))
                    data.append(box)


        data = sorted(data)  #需要对数据进行排序，否则下一步将不会得出正确结果，按照文件名来进行排序，data是一个二维数据，每行表示一张图片，每行分别存储着该图片的图片名、类别、坐标
        # 图片排序的原因：一张图片中对应这多个anchorbox边界框，
        # 所以在下面整理实际样本和标签清单中是一张图片一张图片的读取，
        # 即读取完上一张图片的所有anchorbox边界框，在读取下一张图片的所有案场orbox边界框。
        # 因此必须按照文件名来进行排序，将不同的anchorbox,但是属于同一张图片的anchorbox放在一起，以便后续按照图片读取。



        #整理实际的样品和标签清单
        current_file = data[0][0] # 保存当前图片的文件名
        current_image_id = data[0][0].split('.')[0] # 保留当前图像名中‘.’之前的部分，去掉后缀
        current_labels = []  # 保存当前图片的类别和坐标信息，一张图片中对应这多个anchorbox边界框
        ...
        if ret: # 这个条件下，我们想返回这些。 本项目中 images = None
    ...
    def parse_json(self,
        ...
        self.dataset_size = len(self.filenames)  #文件中的图片数量
        self.dataset_indices = np.arange(self.dataset_size, dtype=np.int32)  # 文件中图片对应的index值
    ...
    def generate(self,
        ...
        if shuffle:
            objects_to_shuffle = [self.dataset_indices]  # 读取到的图片的index值
            if not (self.filenames is None):  # 文件路径
                objects_to_shuffle.append(self.filenames)
            if not (self.labels is None):  # 坐标和类别信息
                objects_to_shuffle.append(self.labels)
            if not (self.image_ids is None):  # 文件名（不包括后缀）
                objects_to_shuffle.append(self.image_ids)
            if not (self.eval_neutral is None):  # eval_neutral 为None
        ...
        while True:
            ...
            #########################################################################################
            # 获取该批的图像、(可能)图像id、(可能)标签等。.
            #########################################################################################

            # 我们按以下顺序优先考虑我们的选择:
            # 1) 如果我们已经在内存中加载了图像，从那里获取它们.
            # 2) 否则，如果我们有HDF5数据集，从那里获取图像.
            # 3) 否则，如果我们没有上述两个，我们将不得不从磁盘加载单独的图像文件。
            ...
            batch_items_to_remove = [] # 如果我们需要从批处理中删除任何图像，请将它们的索引存储在这个列表中.
            batch_inverse_transforms = []

            for i in range(len(batch_X)):

                if not (self.labels is None):
                    # 将此图像的标签转换为一个数组(以防它们还没有)。
                    batch_y[i] = np.array(batch_y[i])
                    # 如果这个图像没有人工标注框，那么我们可能不想将它保存在批处理中.
            ...
            ret = []
            if 'processed_images' in returns: ret.append(batch_X) # batch_X.shape = (5, 300, 480, 3) 存储图片数据
            ...
            if 'image_ids' in returns: ret.append(batch_image_ids) # batch_image_ids.shape = (5, ) # 存储批处理的图片的图片名（不包括后缀名）
            if 'evaluation-neutral' in returns: ret.append(batch_eval_neutral)  # None
            if 'inverse_transform' in returns: ret.append(batch_inverse_transforms) # 可以暂时理解成数据增强
            if 'original_images' in returns: ret.append(batch_original_images)
            if 'original_labels' in returns: ret.append(batch_original_labels) # batch_original_labels.shape = (5, ),相当于存储批处理的图片的类别和坐标信息， 坐标格式是（xmin, ymin, xmax, ymax）
    ...
    def enhance_data_inform(self):
        raw_labels = []
        for label in self.labels:
            raw_label = []
            for i in label:
                aaa = self.labels_format['class_id']
                if (i[self.labels_format['class_id']] != 1) :
                    raw_label.append(i)
